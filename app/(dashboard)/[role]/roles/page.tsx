...
export default function RolesPage() {
    ...
    return (
        <div className="flex h-[calc(100vh-120px)] gap-6 overflow-hidden p-6">
            {/* Sidebar: Role List */}
            ...
            <Dialog open={showAssignDialog} onOpenChange={setShowAssignDialog}>
                <DialogContent className="rounded-[2rem] max-w-md p-0 overflow-hidden border-none shadow-2xl flex flex-col">
                    <DialogHeader className="p-6 bg-slate-50 border-b border-slate-100">
                        <DialogTitle className="text-xl font-black text-slate-800">Assign User to {selectedRole?.name}</DialogTitle>
                    ...
                    <div className="p-6 flex-1 space-y-4 bg-white">
                        <div className="relative group">
                            ...
                            <Input
                                ...
                                className="pl-11 h-12 rounded-2xl bg-slate-50 border-slate-200 text-sm focus-visible:ring-primary/10 focus-visible:border-primary/30 transition-all font-medium"
                                autoFocus
                            />
                        </div>

                        <ScrollArea className="h-[380px] pr-2 -mr-2">
                            <div className="space-y-2">
                                {filteredAllUsers.length === 0 ? (
                                    ...
                                    filteredAllUsers.map(user => (
                                        <button key={user.id}
                                            ...
                                            className="w-full flex items-center gap-4 p-4 rounded-2xl hover:bg-primary/5 border border-transparent hover:border-primary/10 transition-all text-left group">
                                            <Avatar className="h-11 w-11 rounded-2xl border border-white shadow-sm shrink-0">
                                                <AvatarImage src={user.imageUrl} />
                                                <AvatarFallback className="rounded-2xl bg-primary/10 text-primary font-black text-sm">
                                                    {user.name?.charAt(0).toUpperCase() || "U"}
                                                </AvatarFallback>
                                            </Avatar>
                                            <div className="flex-1 overflow-hidden">
                                                <div className="flex items-center justify-between gap-2">
                                                    ...
                                                    <Badge variant="outline" className={`text-[9px] font-black rounded-lg px-2 uppercase border ${getRoleColor(user.role)}`}>
                                                        {user.role}
                                                    </Badge>
                                                </div>
                                            ...
                                            <div className="h-8 w-8 rounded-xl bg-white border border-border/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all shadow-sm">
