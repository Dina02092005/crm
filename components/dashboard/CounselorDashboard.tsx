...
import { LeadsCharts } from "@/components/dashboard/analytics/LeadsCharts";
import { ApplicationsCharts } from "@/components/dashboard/analytics/ApplicationsCharts";
import { VisaCharts } from "@/components/dashboard/analytics/VisaCharts";
...
export function CounselorDashboard() {
    ...
    const fetchAnalytics = async () => {
        setIsLoading(true);
        try {
            const response = await axios.get("/api/analytics/counselor");
            setData(response.data);
        } catch (error) {
            console.error("Failed to fetch counselor analytics", error);
    ...
    return (
        <div className="flex flex-col gap-6 p-4 sm:p-6 max-w-[1600px] mx-auto w-full">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                        Your personal performance and assigned workflow overview
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                {/* Left (2/3): Personal Performance Charts */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <Card className="rounded-2xl bg-card border border-border shadow-none">
                            <CardHeader className="pb-2">
                                <CardTitle className="text-sm font-semibold text-foreground/80">My Student Funnel</CardTitle>
                            </CardHeader>
                            <CardContent>
                ...
                <div className="space-y-6">
                    ...
                    <Card className="rounded-2xl bg-card border border-border shadow-none">
                        <CardHeader className="flex flex-row items-center gap-2 pb-2">
                            ...
                            <CardTitle className="text-sm font-semibold text-foreground/80">My Pending Submissions</CardTitle>
                        </CardHeader>
                        <CardContent>
                            {isLoading ? (
                                <div className="space-y-3">
                    ...
                    <Card className="rounded-2xl bg-card border border-border shadow-none">
                        <CardHeader className="flex flex-row items-center gap-2 pb-2">
                            ...
                            <CardTitle className="text-sm font-semibold text-foreground/80">My Visa Docs Pending</CardTitle>
                        </CardHeader>
                        <CardContent>
                            {isLoading ? (
                                <div className="space-y-3">
